...
void SectionsInfo::reset() {
	radialSections.clear();
    linearSections.clear();
    scatterSections.clear();
    actionQueue.clear();
}
bool SectionsInfo::hasSections() const {
	return !radialSections.empty() || !linearSections.empty() || !scatterSections.empty();
}
...
std::vector<RadialSection> generateRadialSections(SectionGenerationInfo generationInfo,
	...
	auto predicate = [&](const RayHit2D& r1, const RayHit2D& r2, const RayHit2D& r3)-> bool {
		...
		return std::abs(slope1 - slope2) > generationInfo.raySettings.radialSectionTolerance;
	};

	return generateSectionsBase<RadialSection>(color, generationInfo.shapes, rays, predicate);
}
...
std::vector<LinearSection> generateLinearSections(SectionGenerationInfo generationInfo,
	...
	auto predicate = [&](const RayHit2D& r1, const RayHit2D& r2, const RayHit2D& r3)-> bool {
		real_t slope1 = calculateSlope(r1.location, r2.location);
		real_t slope2 = calculateSlope(r2.location, r3.location);
		return std::abs(slope1 - slope2) > generationInfo.raySettings.linearSectionTolerance;
	};

	return generateSectionsBase<LinearSection>(color, generationInfo.shapes, rays, predicate);
}
...
GenerateScatterSectionResult generateScatterSections(SectionGenerationInfo generationInfo, 
	...
	std::sort(shotResult.rays.begin(), shotResult.rays.end(), 
		[&](const RayVariant& lhs, const RayVariant& rhs) -> bool {
			Point2 lhsOrigin = getRay(lhs).origin;
			Point2 rhsOrigin = getRay(rhs).origin;
	...
	auto predicate = [&](const RayHit2D& r1, const RayHit2D& r2, const RayHit2D& r3)-> bool {
		real_t slope1 = calculateSlope(r1.location, r2.location);
		real_t slope2 = calculateSlope(r2.location, r3.location);
		return std::abs(slope1 - slope2) > generationInfo.raySettings.scatterSectionTolerance;
	};

	std::vector<ScatterSection> scatterSections = generateSectionsBase<ScatterSection>(color, generationInfo.shapes, shotResult.rays, predicate);

	std::vector<RadialSection> postIntersectionSections;
	if(shotResult.behavior == ScatterSectionBehavior::converges) {
		std::optional<RayHit2D> perviousHit;
		for(std::size_t i = 0; i < shotResult.rays.size(); i++) {
			const RayVariant& currentRay = shotResult.rays[i];
			if(std::holds_alternative<RayHit2D>(currentRay)) {
				...
				if(i + 1 < shotResult.rays.size()) {
					const RayVariant& nextRay = shotResult.rays[i + 1];
					if(std::holds_alternative<RayHit2D>(nextRay)) {
						...
						if(nextRayHitPoint.has_value()) {
							...
							postIntersectionSections.push_back(secondShotSection);
	...
	GenerateScatterSectionResult result;
	result.sections = std::move(scatterSections);
	result.postIntersectionSections = std::move(postIntersectionSections);
	return result;
}
...
std::vector<RadialSection> generatePostIntersectionSections(SectionGenerationInfo generationInfo, 
	...
	return generateRadialSections(generationInfo, color, angle, rays);
}